...
class TTAPI {
    ...
    function _post_registerUser() {
        ...
        if (!$existingEmail or is_null($existingEmail['idUser'])) {
            ...
            $userId = createUser($firstName, $lastName, $password, $birthday, $country, $city, $street, $zipcode, $phone, $interests);
            if ($userId) {
    ...
    function _post_login() {
        ...
        if ($type == "normal") {
            ...
            $firstName = filter_input(INPUT_POST, 'firstName');
            $lastName = filter_input(INPUT_POST, 'lastName');
            $personalImage = filter_input(INPUT_POST, 'personalImage');
            $result = socialLoginUser($email, $firstName, $lastName, $personalImage);           
        }
    ...
    function _post_resetPassword() {
        $email = filter_input(INPUT_POST, 'email');
        $existingEmail = readEmail($email);
        $idUser = $existingEmail['idUser'];
        $code = $existingEmail['code'];
        $idEmail = $existingEmail['idEmail'];

        $password = sha1($this->createRandomPassword(16));
        ...
        // $message = "Hello $firstName $lastName \r Your email has been registered to sportsrush.ch . \r To verify that this email belongs to you please click on the link below : \r https://dev.sportsrush.ch/api/verifyEmail?code=$code&idEmail=$idEmail";
        // mail($email, 'Sportsrush Reset password', $message);
    ...
    function _post_updateUser() {
        ...
        $updateSuccess = updateUser($idUser, $firstName, $lastName, $password, $birthday, $country, $city, $interests);
        $dataUser = readUser($idUser);
        $existingEmail = readEmail($dataUser['idUser']);        
    ...
    function _post_createTeam() {        
        ...
        $teamDesc = filter_input(INPUT_POST, 'description');
        $idUser = filter_input(INPUT_POST, 'idUser');
        $type = filter_input(INPUT_POST, 'type');

        $idTeam = createTeamNoLogo($teamName, $teamDesc, $sport, $type);
