...
// CREATE
function createUser($firstName, $lastName, $pwd) {
    $db = getDatabaseObject();
    $query = $db->prepare('
            INSERT INTO T_Users (firstName, lastName, password) 
                    VALUES(:first, :last, :pwd)');
    $query->execute(array(
        'first' => $firstName,
        'last' => $lastName,
        'pwd' => $pwd
...
// UPDATE
function updateUser($idUser, $firstName, $lastName) {
    $db = getDatabaseObject();
...
function hasTeams($idUser) {
    ...
    $query = $db->prepare("
            SELECT T_Memberships.idTeam, teamName, description, coach
            FROM T_Memberships
